...
#region Get PFX path
    ...
    if (-not (Test-Path $PfxPath)) {
        Write-Log -Action "PFX file not found at path: $PfxPath" -Status "ERROR"
        throw "PFX file does not exist. Cannot continue."
    }

    Write-Log -Action "PFX file validated: $PfxPath"
    Write-Host "`tSUCCESS! $PfxPath file validated." -ForegroundColor Green
#endregion
...
#region Load PFX and get password
    ...
    try {
        ...
        throw "`tCould not load certificate."
    }
#endregion
...
#region Find user in Entra using certificate email attribute
    ...
    ## although if a cert is generated for shared users, the email attribute may be missing from the cert.
    ...
    if ($E) {
        ...
        try {
            ...
            Write-Log -Action "Entra ID email lookup failed" -Status "ERROR" -ErrorMessage $_.Exception.Message
            #throw "`tERROR! Error occurred while searching Entra ID by email."
        }
    }
    else {
    ...
    while (-not $importSuccess) {
        ...
        #region resource group picker
            ...
            while (-not ($selection -as [int]) -or $selection -lt 1 -or $selection -gt $RGs.Count) {
                ...
                Write-Log -Action "admin selected resource group $($chosenRG)"
            }
        #endregion
        ...
        Write-Host "`tVaults:" -ForegroundColor Yellow
        $vaultlist.VaultName | foreach-object {write-host `t$_ -ForegroundColor cyan}
        write-host "`nWhich vault do you want to upload this cert to? Enter the name exactly how it appears in the list:" -ForegroundColor Magenta
        $kvName=read-host
        $kvName=$vaultlist | where-object {$_.vaultname -match "^$kvname$"}
...
#region apply tags
    ...
    write-host "`nEnter Which application will this be used for (OATI or CAISO):" -ForegroundColor Magenta
    $tagAppValue=Read-Host
    ...
    try {
        ...
        Write-Log -Action "user tag '$taguservalue' and app tag '$atagappvalue' applied."
    }
    catch {
    ...
    if (-not (Get-AzRoleAssignment -scope $scope -signinname $($MatchedUser.UserPrincipalName) -RoleDefinitionName $userAKVrole)) {
        try{
    ...
    elseif ((Get-AzRoleAssignment -scope $scope -signinname $($MatchedUser.UserPrincipalName) -RoleDefinitionName $userAKVrole)) {
    ...
    if (-not (Get-AzRoleAssignment -scope $secretscope -signinname $($MatchedUser.UserPrincipalName) -RoleDefinitionName $userAKVrole)) {
        try {
    ...
    elseif ((Get-AzRoleAssignment -scope $secretscope -signinname $($MatchedUser.UserPrincipalName) -RoleDefinitionName $userAKVrole)) {
...
try {
    ...
    write-host "`tSUCCESS! deleted $pfxpath." -ForegroundColor Green
    Write-Log -Action "deleted $pfxpath file."
}
catch {
    write-log -action "we couldn't delete $pfxpath" -Status "ERROR" -ErrorMessage $_.Exception.Message
    throw "`twe couldn't delete $pfxpath"
}
clear-variable pfxpath -ErrorAction SilentlyContinue
Clear-Variable PfxPasswordSecure -ErrorAction SilentlyContinue
#endregion
