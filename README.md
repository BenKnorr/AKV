...
9) Customer supplied sample certificates include "CN" attributes that will never match a user (a shared certificate for instance), and some that have similar CN matches to Entra Name attributes. These have been observed to be in a format like "Ben Knorrx12345" ; the script trims the "x[5 digits]" and attempts to match the CN of the certificate to the GivenName and Surname ("Ben Knorr") of EntraID users.
10) If the script can't find a match for Cert:CN to EntraID:"GivenName Surname" , it stops and asks the admin for a manual input for the targeted user. This needs to be in UPN format (bknorr@compunet.biz) of an actual EntraID user.
